...
const DEFAULT_GLOW_COLOR = '201, 169, 75'; // Gold luxury theme matching #C9A94B
...
const updateCardGlowProperties = (card: any, mouseX: number, mouseY: number, glow: number, radius: number) => {
...
export const MagicBentoGrid = ({ children, className = '' }: any) => {
...
export const MagicBentoCard = ({
  ...
  const memoizedParticles = useRef<any[]>([]);
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
      ...
      if (enableTilt) {
        ...
        if (innerRef.current) {
           gsap.to(innerRef.current, { x: 0, y: 0, scale: 1.03, duration: 0.3, ease: 'power2.out' });
        }
      }
    };
    ...
    const handleMouseLeave = () => {
      ...
      if (enableTilt) {
        gsap.to(element, { x: 0, y: 0, duration: 0.3, ease: 'power2.out' });
        if (innerRef.current) {
           gsap.to(innerRef.current, { x: 0, y: 0, scale: 1, duration: 0.3, ease: 'power2.out' });
        }
      }
    ...
    const handleMouseMove = (e: any) => {
      ...
      if (enableTilt) {
        ...
        gsap.to(element, { x: mx, y: my, duration: 0.1, ease: 'power2.out' });
        if (innerRef.current) {
          gsap.to(innerRef.current, { x: -mx, y: -my, scale: 1.03, duration: 0.1, ease: 'power2.out' });
        }
      }
  ...
  }, [animateParticles, clearAllParticles, shouldDisableAnimations, enableTilt, enableMagnetism, clickEffect, glowColor, enableStars, onClick]);
  ...
  return (
    <div
      ...
      onClick={!clickEffect ? onClick : undefined} // Only attach directly if clickEffect is disabled (to avoid double firing)
    >
      <div ref={innerRef} className="h-full w-full relative z-10 transition-transform duration-100 ease-out">
        {children}
      </div>
    </div>
  );
